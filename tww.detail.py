...
for line in lines:
    ...
    if not os.path.isdir("profiles/tww/"+index+name):
        os.makedirs("profiles/tww/"+index+name)

    if os.path.exists("profiles/tww/"+index+name+"/"+index+name+".txt"):
        pfile = open("profiles/tww/"+index+name+"/"+index+name+".txt", 'r')
        plines=pfile.readlines()
        if len(plines)==0:
            os.remove("profiles/tww/"+index+name+"/"+index+name+".txt")

    if not os.path.exists("profiles/tww/"+index+name+"/"+index+name+".txt"):
        while True:
            try:
                driver.get(url)
                break
        ...
        pfile = open("profiles/tww/" + index + name + "/" + index + name + ".txt", 'w')
