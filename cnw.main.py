...
while emptyData=="display: block":
    driver.find_element(By.CSS_SELECTOR,"div[aria-label*='内地']").click();
    time.sleep(5)
...
the_file=open('urls/cn.woman.txt', 'w')
count=0
print("--------第一页------------")
...
for starE in starElements:
    ...
    line="cnw"+str(count)+"|"+starE.get_attribute("title")+"|"+starE.get_attribute("href")+"|"+starImg.get_attribute("src")+"\n"
    print(line)
    the_file.write(line)
...
for pageIndex in range(2,1500):
    ...
    while emptyData == "display: block;":
        ...
        for starE in starElements:
            ...
            line = "cnw"+str(count) + "|" + starE.get_attribute("title") + "|" + starE.get_attribute(
