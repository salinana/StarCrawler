...
while emptyData=="display: block":
    driver.find_element(By.CSS_SELECTOR,"div[aria-label*='香港']").click();
    time.sleep(5)
    driver.find_element(By.CSS_SELECTOR,"div[aria-label*='男']").click();
    time.sleep(5)
    emptyData=driver.find_element(By.CSS_SELECTOR,"div[id='emptyData']").get_attribute("style")
    print(emptyData)

the_file=open('urls/hk.man.txt', 'w')
count=0
print("--------第一页------------")
...
for starE in starElements:
    ...
    line="hkm"+str(count)+"|"+starE.get_attribute("title")+"|"+starE.get_attribute("href")+"|"+starImg.get_attribute("src")+"\n"
    print(line)
    the_file.write(line)
...
for pageIndex in range(2,1500):
    ...
    while emptyData == "display: block;":
        ...
        for starE in starElements:
            ...
            line = "hkm"+str(count) + "|" + starE.get_attribute("title") + "|" + starE.get_attribute(
