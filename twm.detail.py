...
for line in lines:
    ...
    if not os.path.isdir("profiles/twm/"+index+name):
        os.makedirs("profiles/twm/"+index+name)

    if os.path.exists("profiles/twm/"+index+name+"/"+index+name+".txt"):
        pfile = open("profiles/twm/"+index+name+"/"+index+name+".txt", 'r')
        plines=pfile.readlines()
        if len(plines)==0:
            os.remove("profiles/twm/"+index+name+"/"+index+name+".txt")

    if not os.path.exists("profiles/twm/"+index+name+"/"+index+name+".txt"):
        while True:
            try:
                driver.get(url)
                break
        ...
        pfile = open("profiles/twm/" + index + name + "/" + index + name + ".txt", 'w')
