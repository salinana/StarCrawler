...
for line in lines:
    ...
    if not os.path.isdir("profiles/hkw/"+index+name):
        os.makedirs("profiles/hkw/"+index+name)

    if os.path.exists("profiles/hkw/"+index+name+"/"+index+name+".txt"):
        pfile = open("profiles/hkw/"+index+name+"/"+index+name+".txt", 'r')
        plines=pfile.readlines()
        if len(plines)==0:
            os.remove("profiles/hkw/"+index+name+"/"+index+name+".txt")

    if not os.path.exists("profiles/hkw/"+index+name+"/"+index+name+".txt"):
        while True:
            try:
                driver.get(url)
                break
        ...
        pfile = open("profiles/hkw/" + index + name + "/" + index + name + ".txt", 'w')
