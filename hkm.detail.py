...
for line in lines:
    ...
    if not os.path.isdir("profiles/hkm/"+index+name):
        os.makedirs("profiles/hkm/"+index+name)

    if os.path.exists("profiles/hkm/"+index+name+"/"+index+name+".txt"):
        pfile = open("profiles/hkm/"+index+name+"/"+index+name+".txt", 'r')
        plines=pfile.readlines()
        if len(plines)==0:
            os.remove("profiles/hkm/"+index+name+"/"+index+name+".txt")

    if not os.path.exists("profiles/hkm/"+index+name+"/"+index+name+".txt"):
        while True:
            try:
                driver.get(url)
                break
        ...
        pfile = open("profiles/hkm/" + index + name + "/" + index + name + ".txt", 'w')
