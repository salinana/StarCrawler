...
for line in lines:
    ...
    if not os.path.isdir("profiles/cnm/"+index+name):
        os.makedirs("profiles/cnm/"+index+name)

    if os.path.exists("profiles/cnm/"+index+name+"/"+index+name+".txt"):
        pfile = open("profiles/cnm/"+index+name+"/"+index+name+".txt", 'r')
        plines=pfile.readlines()
        if len(plines)==0:
            os.remove("profiles/cnm/"+index+name+"/"+index+name+".txt")

    if not os.path.exists("profiles/cnm/"+index+name+"/"+index+name+".txt"):
        while True:
            try:
                driver.get(url)
                break
        ...
        pfile = open("profiles/cnm/" + index + name + "/" + index + name + ".txt", 'w')
